...
# our_quant = 16 if 16 bits quantization and 32 if no quantization
our_quant=16
...
def profile(model, input_size, custom_ops = {}):
    ...
    def add_hooks(m):
        ...
        for p in m.parameters():
            non_zero_params =(p.view(-1)!= 0).sum().item()#compute the non zero params
            m.total_params += torch.Tensor([non_zero_params]) / quant_factor # Score changes with quantification            
...
def main():
    ...
    ref_params = 11173962.0
    
    ref_flops  = 1112074240.0
    
    # WideResnet-28-10 - Reference for CIFAR 100
    # ref_params = 36500000'''model249_mixup_Pruning_depruned_quantized.pt
